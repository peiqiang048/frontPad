				...
				<view class="account-left-row1-1">
					...
					<view class="account-font-size-small account-display" style="flex: none;margin: auto 10px;">
						...
						<view class="leftQuanText">
							<!-- 多语言 - 未结算 -->
							{{i18n.orderAccount.unsettled}}
						</view>
						<!-- 右侧红色圈 -->
						<view class="account-display" style="margin-left: 20px;">
							<!-- 红色圈图片 -->
							<image class="leftQuanImg" :src="leftRedQuan"></image>
						</view>
						<!-- 已结算文本 -->
						<view class="leftQuanText">
							<!-- 多语言 - 已结算 -->
							{{i18n.orderAccount.settled}}
						</view>
					</view>
				</view>
			</view>
			<!-- 底部白色背景部分：订单信息 -->
			<scroll-view :scroll-top="orderItemListScroll" scroll-y="true" class="account-left-row2" id="orderItemList" @scroll="scrollfunc"
			 v-bind:style="{ height: leftCenterHeight + 'px' }">
				<block v-for="(item ,index) in orderItemList" :key="index">
					<!-- 一行菜品信息(绿色圈未结算菜品) 宽度100% 高度自适应 -->
					<view class="account-left-row2-1">
						<view class="account-left-row-itemOption">
							<!-- 菜品名 -->
							...
							<!-- 菜品价格 -->
							<view v-if="item.itemClassification !== 0 && item.itemClassification !== '0'" class="account-item-money">
								￥-{{item.itemPrice | thousandthSign}}
							</view>
							<view v-else class="account-item-money">
								￥{{ item.itemPrice | thousandthSign}}
							</view>
						</view>
						<view class="account-left-row-itemOption">
			...
			<view class="account-right-row2" v-bind:style="{ height: rightCenterHeight + 'px' }">
				...
				<view class="account-pay-right">
					...
					<view class="account-pay-money account-display">
						...
						<view class="account-hidden" style="float: left;flex: 1;">
							{{(parseFloat(this.payInAmount === '' ? 0 : this.payInAmount) - parseFloat(this.orderUncollectedAmount)) < 0 ? 0 : (parseFloat(this.payInAmount === '' ? 0 : this.payInAmount) - parseFloat(this.orderUncollectedAmount)) | thousandthSign}}
						</view>
					</view>
					<!-- QRコードPAY -->
					<view v-if="paymentTypeIsActive === '98' || (paymentTypeIsActive >= '21' && paymentTypeIsActive <= '36')" class="QRPay">
						...
						<input @input="inputChange" maxlength="500" :value="paymentCodeQR" class="QRPayCenter" />
...
<script>
	...
	export default {
		...
		onLoad: function(option) {
			...
			if (this.languages === 'zh' || this.languages === 'ko') {
				// 界面字体大小 （整单折扣，整单让价）
				this.fontSize = '2vw';
			} else if (this.languages === 'ja' || this.languages === 'en') {
				// 界面字体大小 （整单折扣，整单让价）
				this.fontSize = '1.5vw';
			}
		...
		methods: {
			...
			scrollfunc: function(e) {
				this.orderItemListScroll = e.detail.scrollTop;
				if (this.orderItemListScroll > 0) {
					// 滚动条滚动显示底部提示
					this.scrollBottomTips = true;
				}
			},
			// 窗口尺寸计算
			...
			confirmButton: function() {
				...
				if (this.isAccount === 1) {
					...
					if (Number(this.payInAmount) === Number(this.orderUncollectedAmount)) {
						uni.reLaunch({
				...
				http
					...
					.then(result => {
						...
						if (result.data.resultCode === '0000') {
							...
							// 判断割引额是否为空，不是的话，代表这笔订单不是第一次结算
							if (this.priceDiscountAmount !== null && this.priceDiscountAmount !== '' &&
								this.priceDiscountAmount !== undefined) {
								// 计算优惠金额
								this.orderAmountDiscount = this.priceDiscountAmount;
							}
							// 判断割引率是否为空，不是的话，代表这笔订单不是第一次结算
							if (this.priceDiscountRate !== null && this.priceDiscountRate !== '' &&
								this.priceDiscountRate !== undefined) {
								// 计算优惠金额
								this.orderAmountDiscount = Math.round(parseFloat(this.total) * (parseFloat(this.priceDiscountRate) / 10));
							}
							...
							if (result.data.takeoutFlag === '') {
								...
								this.foreignTaxSwithStyleL = '#3B99FC';
								...
								if (this.foreignTaxSwith === 0 || this.foreignTaxSwith === '0') {
									...
									this.foreignTaxSwithStyleR = '#3B99FC';
								}
							}
			...
			toCheckOut: function() {
				...
				if (parseFloat(this.payInAmount) <= 0) {
					// 结算金额 大于 未收金额 -- '请核对结算金额'
					this.tipShow(this.i18n.orderAccount.messageC, true, '2', '1');
					return;
				}
				// お釣
				this.paymentChange = (parseFloat(this.payInAmount === '' ? 0 : this.payInAmount) - parseFloat(this.orderUncollectedAmount)) < 0 ? 0 : (parseFloat(this.payInAmount === '' ? 0 : this.payInAmount) - parseFloat(this.orderUncollectedAmount));
				//預かり金額
				...
				if (this.paymentTypeIsActive === '98' || ('21' <= this.paymentTypeIsActive && this.paymentTypeIsActive <= '36')) {
					if (this.paymentCodeQR !== '') {
						...
						// QRコードPAY结账
						http
							.post('api/stpd/payLater', that.dataParameter)
							.then(result => {
								if (result.data.resultCode === '0000') {
					...
					// 现金结账
					http
						.post('api/stpd/registPayment', that.dataParameter)
						.then(result => {
							if (result.data.resultCode === '0000') {
								this.isAccount = 1;
								printModule.printSlip(
								...
								// 结算成功
								this.tipShow(this.paymentChangeMessage.replace('{0}', this.moenySign(this.paymentChange)), true, '2', '2');
			...
			discountShowClick: function() {
				// 判断值引额和值引率是否为空，都为空代表是没结算过的订单，但凡有一个不为空，就说明已经结算过了，那么就不能显示折扣画面
				if ((this.priceDiscountAmount === null || this.priceDiscountAmount === '' || this.priceDiscountAmount ===
						undefined) && (this.priceDiscountRate === null || this.priceDiscountRate === '' || this.priceDiscountRate ===
						undefined)) {
					if (!this.buttonStatus) {
						// 参数错误，按钮失效
						return;
			...
			discountSure: function() {
				...
				if (this.discountAmount !== '') {
					...
					// 优惠信息输入错误提示 -- '请输入正确的优惠信息'
					// this.tipShow(this.i18n.orderAccount.messageE, true, '2');
					// 计算未收金额
			...
			foreignTaxSwithClick: function(e) {
				...
				if (this.buffetId !== null && this.buffetId !== undefined && this.buffetId !== '') {
					// 放题时就餐方式不能切换
					return;
				}
